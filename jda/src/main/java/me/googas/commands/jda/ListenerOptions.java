...
public interface ListenerOptions {
   ...
   */
  Message processResult(Result result, @NonNull GenericCommandContext context);

  /**
   * Process the consumer of the message from {@link #processResult(Result, GenericCommandContext)},
   * this means providing the {@link me.googas.commands.jda.listener.CommandListener} with a result
   * for the message sent in the {@link net.dv8tion.jda.api.entities.TextChannel} this can be used
   * to delete the message after a few seconds if desired like the implementation {@link
   ...
   */
  Consumer<Message> processConsumer(Result result, @NonNull GenericCommandContext context);
   ...
   * @param context the context of the command execution
   */
  void handle(@NonNull Throwable fail, @NonNull GenericCommandContext context);
